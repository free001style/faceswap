...
def vis_parsing_maps(image, parsing_anno, stride=1):
    ...
    return Image.fromarray(vis_im.astype('uint8'))
...
class FaceParser(nn.Module):
    ...
    def preprocess_img(self, img):
        img_orig = img
        if img_orig.shape[1] >= 512:
            im = img_orig[:3].unsqueeze(0).to(self.device)
            im = normalize(self.downsample(im).clamp(0, 1))
        else:
...
def faceParsing_demo(model, img, convert_to_seg12=True, model_name="default"):
    ...
    with torch.no_grad():
        if model_name == "default":
            ret = np.zeros((img.shape[0], 512, 512))
            for i in range(img.shape[0]):
                seg = model(img[i]).cpu().numpy().astype(np.uint8)
                if convert_to_seg12:
