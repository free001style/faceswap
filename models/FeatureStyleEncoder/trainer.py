...
class Trainer(nn.Module):
    def __init__(self, config, opts, device):
        ...
        self.enc = fs_encoder_v2(n_styles=self.n_styles, opts=opts, residual=enc_residual, use_coeff=enc_residual_coeff,
                                 resnet_layer=resnet_layers, stride=self.stride)
        ...
        if 'optimizer' in self.config and self.config['optimizer'] == 'ranger':
            self.enc_opt = Ranger(self.enc_params, lr=config['lr'], betas=(config['beta_1'], config['beta_2']),
                                  weight_decay=config['weight_decay'])
        else:
            self.enc_opt = torch.optim.Adam(self.enc_params, lr=config['lr'],
                                            betas=(config['beta_1'], config['beta_2']),
                                            weight_decay=config['weight_decay'])
        self.enc_scheduler = torch.optim.lr_scheduler.StepLR(self.enc_opt, step_size=config['step_size'],
                                                             gamma=config['gamma'])
    ...
    def initialize(self, stylegan_model_path, arcface_model_path, parsing_model_path):
        # load StyleGAN model
        stylegan_state_dict = torch.load(stylegan_model_path)
        self.StyleGAN.load_state_dict(stylegan_state_dict['g_ema'], strict=False)
        ...
        self.noise_inputs = [getattr(self.StyleGAN.noises, f'noise_{i}') for i in
                             range(self.StyleGAN.num_layers)]
        # load Arcface weight
        self.Arcface.load_state_dict(torch.load(self.opts.arcface_model_path))
        self.Arcface.eval()
        # load face parsing net weight
        # self.parsing_net.load_state_dict(torch.load(self.opts.face_parsing_path))
    ...
    def landmarkloss(self, input, target):
        ...
        parsing_loss = sum(
            [1 - cos(out_1[i].flatten(start_dim=1), out_2[i].flatten(start_dim=1)) for i in range(len(out_1))])
    ...
    def save_image(self, log_dir, n_epoch, n_iter, prefix='/scripts/', w=None, img=None, noise=None,
                   training_mode=True):
        return self.save_image_stylegan2(log_dir=log_dir, n_epoch=n_epoch, n_iter=n_iter, prefix=prefix, w=w, img=img,
                                         noise=noise, training_mode=training_mode)

    def save_image_stylegan2(self, log_dir, n_epoch, n_iter, prefix='/scripts/', w=None, img=None, noise=None,
                             training_mode=True):
        ...
        with torch.no_grad():
            ...
            utils.save_image(clip_img(out_img[:1]),
                             log_dir + prefix + 'epoch_' + str(n_epoch + 1) + '_iter_' + str(n_iter + 1) + '_0.jpg')
            if out_img.size(0) > 1:
                utils.save_image(clip_img(out_img[1:]),
                                 log_dir + prefix + 'epoch_' + str(n_epoch + 1) + '_iter_' + str(n_iter + 1) + '_1.jpg')
