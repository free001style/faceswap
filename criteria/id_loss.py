...
#         loss_all = [0] * y.shape[0]

#         for j, (y_hat_feats, y_feats) in enumerate(zip(y_hat_feats_ms, y_feats_ms)):
...
#                 loss_all[i] += 1 - sim_target

#             # loss_all += loss
         
#         return torch.tensor(loss_all)

from criteria.iresnet import iresnet100
from utils.torch_utils import get_keys
...
class IDLoss(nn.Module):
    def __init__(self, opts):
        ...
        self.face_pool_1 = torch.nn.AdaptiveAvgPool2d((256, 256))
        self.face_pool_2 = torch.nn.AdaptiveAvgPool2d((112, 112))
        self.facenet = iresnet100(pretrained=False, fp16=False)
        ckpt = torch.load('pretrained_ckpts/blendswap.pth')
        self.facenet.load_state_dict(get_keys(ckpt, "Z_e.1"))
        # self.facenet = Backbone(input_size=112, num_layers=50, drop_ratio=0.6, mode='ir_se')
        # self.facenet.load_state_dict(torch.load(opts.ir_se50_path))
        self.facenet.eval()
    ...
    def extract_feats(self, x):
        ...
        # x_feats = self.facenet(x, multi_scale=False)
        return x_feats
    ...
    def forward(self, y_hat, y):
        ...
        # y_feat = self.extract_feats(y)[0]
        # y_hat_feat = self.extract_feats(y_hat)[0]
        return 1 - torch.torch.nn.functional.cosine_similarity(y_feat, y_hat_feat, dim=1)
