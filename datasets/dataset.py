...
class CelebAHQDataset(Dataset):
    def __init__(self, dataset_root, mode="test",
        ...
        if mode == "train":
            ...
            self.imgs_s = sorted(
                [os.path.join('celeba', "source", "%d.jpg" % idx) for idx in range(1000)])
            self.imgs_t = sorted(
                [os.path.join('celeba', "target", "%d.jpg" % idx) for idx in range(1000)])
        self.imgs_s = self.imgs_s[:int(len(self.imgs_s) * self.fraction)]

        # image pairs indices
        self.indices = np.arange(len(self.imgs_s))
    ...
    def load_single_image(self, index):
        ...
        """
        img1 = self.imgs_s[index]
        img2 = self.imgs_t[index]
...
class FFHQDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img1, img2, flag = self.load_single_image(index)
