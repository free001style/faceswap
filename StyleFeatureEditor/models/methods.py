...
# from StyleFeatureEditor.utils.class_registry import ClassRegistry
from StyleFeatureEditor.utils.common_utils import get_keys
from StyleFeatureEditor.utils.model_utils import toogle_grad
from StyleFeatureEditor.configs.paths import DefaultPaths
from argparse import Namespace
# from StyleFeatureEditor.training.loggers import BaseTimer
...
class FSEInverter(nn.Module):
    ...
    def load_weights(self):
        ...
        print("Loading decoder from", self.opts.stylegan_weights)
        ckpt = torch.load("pretrained_ckpts/stylegan2/stylegan2-ffhq-config-f.pt")
        self.decoder.load_state_dict(ckpt["g_ema"], strict=False)
        self.latent_avg = ckpt['latent_avg'].to(self.device)
...
# @methods_registry.add_to_registry("fse_inverter32", stop_args=("self", "checkpoint_path"))
class FSEInverter32(FSEInverter):
    def set_encoder(self):
        inverter = psp_encoders.Inverter32(opts=self.opts, n_styles=18)
        return inverter  # trainable part
    ...
    def forward(self, x, return_latents=False, n_iter=1e5):
        ...
        w_feat = w_feats[7] 
        fused_feat = self.encoder.fuser(torch.cat([predicted_feat, w_feat], dim=1))
        # feats = [None] * 7 + [fused_feat] + [None] * (17 - 7)
        ...
        #     return_features=True,
        #     new_features=feats,
        #     feature_scale=min(1.0, n_iter * 0.0001),
        #     is_stylespace=False,
        #     randomize_noise=False
        # )

        # return w_recon, fused_feat
        return w_recon, fused_feat
